...
WITH t1 AS (SELECT (first_name || ' ' || last_name) AS name, 
	...
	t3 AS (SELECT t1.name,
	   				...
	   				LEAD(SUM(t1.amount)) OVER (PARTITION BY t1.name ORDER BY DATE_PART('month', t1.payment_date)) - SUM(t1.amount) AS lead_diff
  			FROM t1
       		JOIN t2
...
SELECT t3.*,
	CASE
		WHEN t3.lead_diff = (SELECT MAX(t3.lead_diff) FROM t3 ORDER BY 1 DESC LIMIT 1) THEN 'this is the max diff'
		ELSE NULL
		END AS is_max
	FROM t3
ORDER BY 1